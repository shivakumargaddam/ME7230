...
while true
    ...
    Jacob = zeros(ndof*numnode,ndof*numnode);
    ...
    for iel = 1:numelem
        ...
        for igp = 1:size(w,2)
            ...
            % Jacobian
            Jacob(gind,gind) = Jacob(gind,gind) + (-dnx'*Area*Eo*alpha*duk*dnx + dnx'*Area*E_u*dnx)*w(igp)*det(jac) ;
    ...
    %     temp = temp + Jacob(:,ebcdof(in))*ebcval(in) ;
    % end
    ...
    Jacobmod = Jacob(activeDof,activeDof) ;
    
    % solve
    del_u = Jacobmod\-resmod(activeDof,1);
    
    % has to be supplemented with the boundary conditions
    udisp(ebcdof) = ebcval;
    udisp(activeDof,1) = uguess(activeDof,1) + del_u ;
